//===- Dialect.cpp - Tiny IR Dialect registration in MLIR ------------------===//
//===----------------------------------------------------------------------===//
//
// This file implements the dialect for the Tiny IR: custom type parsing and
...
#include "tiny/Dialect.cpp.inc"

//===----------------------------------------------------------------------===//
// TinyInlinerInterface
//===----------------------------------------------------------------------===//

/// This class defines the interface for handling inlining with Tiny
/// operations.
struct TinyInlinerInterface : public DialectInlinerInterface {
  ...
  /// All call operations within tiny can be inlined.
  bool isLegalToInline(Operation *call, Operation *callable,
                       bool wouldBeCloned) const final {
    return true;
  }

  /// All operations within tiny can be inlined.
  bool isLegalToInline(Operation *, Region *, bool,
                       BlockAndValueMapping &) const final {
    return true;
  }

  // All functions within tiny can be inlined.
  ...
  /// Handle the given inlined terminator(tiny.return) by replacing it with a new
...
namespace tiny {
namespace detail {
/// This class represents the internal storage of the Tiny `StructType`.
...
/// Parse an instance of a type registered to the tiny dialect.
mlir::Type TinyDialect::parseType(mlir::DialectAsmParser &parser) const {
...
/// Print an instance of a type registered to the tiny dialect.
void TinyDialect::printType(mlir::Type type,
                           mlir::DialectAsmPrinter &printer) const {
  // Currently the only tiny type is a struct type.
...
#include "tiny/Ops.cpp.inc"
      >();
  addInterfaces<TinyInlinerInterface>();
  addTypes<StructType>();
}

mlir::Operation *TinyDialect::materializeConstant(mlir::OpBuilder &builder,
