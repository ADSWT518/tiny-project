//===- MLIRGen.cpp - MLIR Generation from a Tiny AST -----------------------===//
//===----------------------------------------------------------------------===//
//
...
/// Implementation of a simple MLIR emission from the Tiny AST.
///
/// This will emit operations that are specific to the Tiny language, preserving
...
public:
  ...
  /// Public API: convert the AST for a Tiny module (source file) to an MLIR
  /// Module operation.
  mlir::ModuleOp mlirGen(ModuleAST &moduleAST) {
    ...
    for (auto &record : moduleAST) {
      if (FunctionAST *funcAST = llvm::dyn_cast<FunctionAST>(record.get())) {
        mlir::tiny::FuncOp func = mlirGen(*funcAST);
        if (!func)
          return nullptr;
...
private:
  /// A "module" matches a Tiny source file: containing a list of functions.
  ...
  llvm::StringMap<mlir::tiny::FuncOp> functionMap;
  ...
  /// Helper conversion for a Tiny AST location to an MLIR location.
  ...
  /// provided Tiny AST prototype.
  mlir::tiny::FuncOp mlirGen(PrototypeAST &proto) {
    ...
    return builder.create<mlir::tiny::FuncOp>(location, proto.getName(),
                                             funcType);
  }

  /// Emit a new function and add it to the MLIR module.
  mlir::tiny::FuncOp mlirGen(FunctionAST &funcAST) {
    // Create a scope in the symbol table to hold variable declarations.
    SymbolTableScopeT varScope(symbolTable);

    // Create an MLIR function for the given prototype.
    builder.setInsertionPointToEnd(theModule.getBody());
    mlir::tiny::FuncOp function = mlirGen(*funcAST.getProto());
    if (!function)
      return nullptr;
  ...
  /// flattened array of data in an Attribute attached to a `tiny.constant`
  ...
  ///   %0 = "tiny.constant"() {value: dense<tensor<2x3xf64>,
  ...
  /// other literals in an Attribute attached to a `tiny.struct_constant`
  ...
  mlir::Value mlirGen(LiteralExprAST &lit) {
    ...
    // Build the MLIR op `tiny.constant`. This invokes the `ConstantOp::build`
    // method.
  ...
  mlir::Value mlirGen(StructLiteralExprAST &lit) {
    ...
    // Build the MLIR op `tiny.struct_constant`. This invokes the
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    if (callee == "transpose") {
      if (call.getArgs().size() != 1) {
        emitError(location, "MLIR codegen encountered an error: tiny.transpose "
    ...
    mlir::tiny::FuncOp calledFunc = calledFuncIt->second;
  ...
  /// Build an MLIR type from a Tiny AST variable type (forward to the generic
